...
public class EchoHandler extends TextWebSocketHandler {
    ...
    protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
        ...
        // Json객체 → Java객체
        // 출력값 : [room_no=123, chatting_no=null, chatting_content=asd, member_id]
        ChattingDto chatMessage = objectMapper.readValue(msg,ChattingDto.class);
        
        // 받은 메세지에 담긴 room_no로 해당 채팅방을 찾아온다. -> chatting인지 room인지
        //ChatRoom chatRoom = cService.selectChatRoom(chatMessage.getRoomId());
        RoomDto chatRoom = roomBiz.selectOne(chatMessage.getRoom_no());
        		
        
        // 채팅 세션 목록에 채팅방이 존재하지 않고, 처음 들어왔고, DB에서의 채팅방이 있을 때
        // 채팅방 생성
        if(RoomList.get(Integer.toString(chatMessage.getRoom_no())) == null && chatMessage.getChatting_content().equals("ENTER-CHAT") && chatRoom != null) {
            ...
            // RoomList에 추가
            RoomList.put(Integer.toString(chatRoom.getRoom_no()), sessionTwo);
            // 확인용
        ...
        // 채팅방이 존재 할 때
            ...
            RoomList.get(Integer.toString(chatMessage.getRoom_no())).add(session);
            // sessionList에 추가
            sessionList.put(session, Integer.toString(chatRoom.getRoom_no()));
            // 확인용
