...
UPDATE CHATTING SET CHATTING_READ = 'Y' 
		WHERE ROOM_NO = 5 AND MEMBER_ID = '머라할까' 
		AND CHATTING_READ IN (SELECT CHATTING_READ FROM CHATTING WHERE CHATTING_READ = 'N')

select * from chatting;

SELECT R.ROOM_NO, M.MEMBER_NICNAME AS MEMBER_ID, R.CONSULT_ID, R.ROOM_CONTENT, R.ROOM_DATE
		FROM ROOM R, MEMBER M
		WHERE R.CONSULT_ID = 'teacher2'
		AND R.MEMBER_ID = M.MEMBER_ID
		ORDER BY R.ROOM_NO
SELECT R.ROOM_NO, R.MEMBER_ID, M.MEMBER_NICNAME AS CONSULT_ID, R.ROOM_CONTENT, R.ROOM_DATE
		FROM ROOM R, MEMBER M
		WHERE R.MEMBER_ID = 'test'
		AND R.CONSULT_ID = M.MEMBER_ID
		ORDER BY R.ROOM_NO
